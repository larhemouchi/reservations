...
class ShowController extends Controller
{
    ...
    */
    ...
    { //pour recuperer une date correcte
    ...
    public function post(Request $request, $id)
    {
        ...
        */
        ...
        if (!$exist)
        { // hna ida kain had id ya3ni mojoud
            /*slug: slug,
            ...
            'id' => $id, 'title' => $request->title, 'slug' => $request->slug, 'poster_url' => $request->poster_url, 'price' => $request->price];

            // hna bach n3erfo wach had location_id exite deja f table locations
            $location_exist = Location::find($request->location_id);

            if ($location_exist)
            { // hna bsara7a mafhamtchi chbaghi te3ml
            ...
            else
            {
                ...
                $arrayLocation = ['id' => $decoded_resp['Venue']['VenueId'], 'phone' => $decoded_resp['Venue']['Telephone'], 'website' => $decoded_resp['Venue']['Email'], 'address' => $decoded_resp['Venue']['Address'], 'designation' => $decoded_resp['Venue']['Name'], 'slug' => str_slug($decoded_resp['Venue']['Name'], '_') ];

                /*****************/

                $locality_exist = Locality::where('locality', $decoded_resp['Venue']['City'])->where('postal_code', $decoded_resp['Venue']['Postcode'])->first();
                ...
                else
                {
                    ...
                    $locality = Locality::create(['locality' => $decoded_resp['Venue']['City'], 'postal_code' => $decoded_resp['Venue']['Postcode']]);
            ...
            if ($show)
            {
                ...
                return response()
                    ->json(['response' => 'no problemme']);
            }
            else
            {
                return response()
                    ->json(['response' => 'problemme f location']);
            }
        }
